...
namespace RFIDPrendas
{
    public partial class FormCaja : Form
    {
        ...
        public FormCaja(FormMain mainForm, RFIDReader pReaderAPI)
        {
            InitializeComponent();
            m_MainForm = mainForm;
        ...
        private void FormCaja_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            m_MainForm.m_FormCaja = null;
        }

        private void retrieveData()
        {
            string ls_Compania = "02";
            string ls_datosPrendas="";
            string ls_OP;
            string ls_corte;
            string ls_subcorte;
            string ls_cod_talla;
            string ls_cod_combinacion;            
            int li_rowscount;
            DataTable ldt_hojas_cajas_nexus;
            DataTable ldt_cajas_mysql;
            BDPrenda prendasBD = new BDPrenda();

            li_rowscount = m_DataTableDatosPrendas.Rows.Count; //verifica la cantidad de etiquetas leídas
            
            //ordena y agrupa las etiquetas 
            var uniqueRows = m_DataTableDatosPrendas.AsEnumerable()
                                  .GroupBy(row => new
                                  {
                                      op = row.Field<string>("op"),
                                      corte = row.Field<string>("corte"),
                                      subcorte = row.Field<string>("subcorte"),
                                      cod_talla = row.Field<string>("cod_talla"),
                                      cod_combinacion = row.Field<string>("cod_combinacion")
                                  })
                                  .Select(group => group.First())
                                  .OrderBy(row => row.Field<string>("op"))
                                  .ThenBy(row => row.Field<string>("corte"))
                                  .ThenByDescending(row => row.Field<string>("subcorte"))
                                  .ThenBy(row => row.Field<string>("cod_talla"))
                                  .ThenByDescending(row => row.Field<string>("cod_combinacion"));

            // Crear un nuevo DataTable con filas únicas
            DataTable uniqueTable = uniqueRows.CopyToDataTable();

            //arma una cadena con los datos de OP, Corte, SubCorte ....
            foreach (DataRow row in uniqueTable.Rows)
            {
                ls_OP = (string)row["op"];
                ls_corte = (string)row["corte"];
                ls_subcorte = (string)row["subcorte"];
                ls_cod_talla = (string)row["cod_talla"];
                ls_cod_combinacion = (string)row["cod_combinacion"];
                string datoprenda = string.Join("|", ls_OP, ls_corte, ls_subcorte, ls_cod_talla, ls_cod_combinacion);
                if (ls_datosPrendas.Length > 0)
                {
                    ls_datosPrendas += ",";
                }
                ls_datosPrendas += datoprenda;
            }
                        
            try
            {
                //recupera las OPs y Hojas de Maracación llamando a store de consulta
                ldt_hojas_cajas_nexus = prendasBD.GetCajasxPrendas(ls_Compania, ls_datosPrendas);

                if (ldt_hojas_cajas_nexus.Rows.Count == 0)
                {
                    dgv_etiquetas.DataSource = ldt_hojas_cajas_nexus;
                }
                else
                {
                    string datosHojaCaja = "";
                    //arma una cadena con todas las cajas registradas 
                    foreach (DataRow row in ldt_hojas_cajas_nexus.Rows)
                    {
                        // Obtener los valores de cada columna en variables
                        string nro_op = Convert.ToString(row["nro_op"]);
                        string nro_hoja_marc = Convert.ToString(row["nro_hoja_marc"]);
                        string nro_caja = Convert.ToString(row["nro_caja"]);
                        string datoCaja = string.Join("|", nro_op, nro_hoja_marc, nro_caja);
                        if (datosHojaCaja.Length > 0)
                            datosHojaCaja += ",";

                        datosHojaCaja += datoCaja;
                    }
                    ...
                    ldt_cajas_mysql = m_ContenedorBD.GetCajas(ls_Compania, datosHojaCaja);
                ...
                foreach (DataGridViewColumn column in dgv_etiquetas.Columns)
                {
                    ...
                    else if (column.HeaderText == "cant_prendas")
                        column.HeaderText = "Cant Prendas";
                    else if (column.HeaderText == "cant_prendas")
                        column.HeaderText = "Cant Prendas";
                    else if (column.HeaderText == "prendas_leidas")
            ...
            if (dgv_etiquetas.Rows.Count <= 1)
            {
                ...
                labelmsj.Text = "No se encontraron registros.";
                uctb_idCaja.Focus();
            }
        }
        ...
        private void FormCaja_SizeChanged(object sender, EventArgs e)
        {
        ...
        private void FormCaja_Shown(object sender, EventArgs e)
        {
            using (FormTrabajador formResponse = new FormTrabajador())
            {
                ...
                {
                    m_Cod_Trabajador = formResponse.CodTrabajador;
                }
                else
                {
