...
namespace RFIDPrendas
{
    class BDHojaMarcacion
    {
        ...
        public DataTable RetrieveOP(string pnro_OP )
        {
            DataTable tabla = new DataTable();
            ...
            try
            {
                using (comando.Connection = myconexion.Connect())
                {
                    ...
                    if (ls_where.Length > 0) ls_where += " AND ";
                    ls_where += $"nnope = @nnope ";
                    comando.Parameters.Add(new AseParameter("@nnope", AseDbType.VarChar)).Value = pnro_OP;
        ...
        public int OPValidar(string nroOP)
        {
            DataTable dtOp = RetrieveOP(nroOP);
            int li_rows = dtOp.Rows.Count;
            if (li_rows == 0)
            {
                return 0;
            }
        ...
        public DataTable RetrieveHojaMarc(string pnro_OP, string pHojaMarc)
        {
            DataTable tabla = new DataTable();
            ...
            try
            {
                using (comando.Connection = myconexion.Connect())
                {
                    string ls_select = $"select norpd, nhjmr, sesthm" +
                    $" from althmc ";
                    ...
                    if (ls_where.Length > 0) ls_where += " AND ";
                    ls_where += $"norpd = @norpd ";
                    if (ls_where.Length > 0) ls_where += " AND ";
                    ls_where += $"nhjmr = @nhjmr ";
                    comando.Parameters.Add(new AseParameter("@norpd", AseDbType.VarChar)).Value = pnro_OP;
                    comando.Parameters.Add(new AseParameter("@nhjmr", AseDbType.VarChar)).Value = pHojaMarc;
        ...
        public int HojaMarcValidar(string nroOP, string pHojaMarc)
        {
            DataTable dtOp = RetrieveHojaMarc(nroOP, pHojaMarc);
            int li_rows = dtOp.Rows.Count;
            if (li_rows == 0)
            {
                return 0;
