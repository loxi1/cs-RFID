...
namespace RFIDPrendas
{
    public partial class FormPrueba : Form
    {
        ...
        public FormPrueba(FormMain mainForm, RFIDReader pReaderAPI, string codTrabajador)
        {
            InitializeComponent();
            m_MainForm = mainForm;
        ...
        private void FormPrueba_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                if (m_MainForm.m_IsConnected)
        ...
        private void retrieveData()
        {
            ...
            ldt_cajas_mysql = m_DBLecturaDeCaja.GetData(dictionary);    //recupera cajas de la tabla contenedor
        ...
        private void FormPrueba_Activated(object sender, EventArgs e)
        {
            updateReaderApi();
            updateGPIState();
        }
        ...
        private void FormPrueba_Shown(object sender, EventArgs e)
        {
            if (m_MainForm.m_IsConnected == false)
            {
            ...
            else
            {
                ...
                try
                {
                    if (m_MainForm.m_IsConnected)
                    {
                        ...
                        readButton.Tag = "Start Reading";
                        m_IsReading = false;
                        SeConEnSegundos.Text = "¡Se contó en tan solo " + tiempo_termina + " seg!";
                    }
                    else
                    {
        ...
        {
            FormIncidente fromIncidente = new FormIncidente(m_Cod_Trabajador,1);
            //fromIncidente.Show();
