    ...
    class DBReportarIncidente
    {
        ...
        public long Insertar(Dictionary<string, object> columns, MySqlConnection myconexion, MySqlTransaction myTransaction)
        {
            ...
            try
            {
                ...
                string tableName = "`incidenterfid`";
        ...
        public DataTable Reporte(string cod_, string date_)
        {
            try
            {
                ...
                comando.Parameters.Clear();
                comando.CommandText = $"SELECT 0 `ORDEN`, `cod_trabajador`, `descripcion`, `fecha_creacion` " +
                    $" FROM `bd_ci_scm`.`incidenterfid` " +
